...
DEFAULT_DB = REPO_ROOT / "data/derived/daily-intelligence/orchestration.db"
DEFAULT_SKILL_PATH = REPO_ROOT / ".agents/skills/fli-daily-intelligence/SKILL.md"
CLI_SCHEMA_VERSION = "1.0"
STORE_SCHEMA_VERSION = "daily-run-store-v1"
RUN_CONTRACT_VERSION = "daily-run-v1"
DEFAULT_CODEX_TIMEOUT_SECONDS = 4 * 60 * 60
DEFAULT_EVIDENCE_WINDOW_DAYS = 9
STAGES = ("evidence", "routing", "prepare", "codex")
...
def connect(path: Path = DEFAULT_DB) -> sqlite3.Connection:
    ...
    conn.executescript(
        ...
        CREATE TABLE IF NOT EXISTS daily_run (
            ...
            UNIQUE(day, config_sha256)
        );
        CREATE INDEX IF NOT EXISTS idx_daily_run_day_updated
            ON daily_run(day, updated_at DESC, run_id);
        """
    )
    row = conn.execute(
        "SELECT schema_version FROM orchestration_meta WHERE singleton = 1"
    ).fetchone()
    if row is None:
        conn.execute(
            "INSERT INTO orchestration_meta(singleton, schema_version) VALUES (1, ?)",
...
def inspect_run(
    *,
    ...
    try:
        if run_id:
            row = conn.execute(
                "SELECT * FROM daily_run WHERE run_id = ?", (run_id,)
            ).fetchone()
        elif day:
            datetime.strptime(day, "%Y-%m-%d")
            row = conn.execute(
                """SELECT * FROM daily_run
...
def _ensure_run(
    ...
    *,
    ...
    row = conn.execute("SELECT * FROM daily_run WHERE run_id = ?", (run_id,)).fetchone()
    if row is not None:
...
def _save_record(
    ...
    *,
    ...
    conn.commit()
    row = conn.execute(
        "SELECT * FROM daily_run WHERE run_id = ?", (record["run_id"],)
    ).fetchone()
    assert row is not None
    return _record_payload(row)
...
def run_day(
    *,
    ...
    plan = {
        ...
        "will_launch_codex": stop_after == "codex",
        "config": config,
    }
    if dry_run:
    ...
    try:
        ...
        if "prepare" not in stages:
            ...
            routing_plan = stages["routing"].get("plan") or []
            expected_routing_id = next(
                (
                    str(item.get("run_id"))
        ...
        if codex.get("goal_status") != "complete":
            record = _save_record(
                ...
                error={
                    ...
                    "retryable": codex.get("goal_status") not in {"budgetLimited", "usageLimited"},
...
def _parser() -> argparse.ArgumentParser:
    ...
    run = sub.add_parser("run", help="Run or resume one date-keyed workflow.")
    ...
    run.add_argument("--stop-after", choices=STAGES, default="codex")
    run.add_argument(
    ...
    inspect = sub.add_parser("inspect", help="Inspect one durable orchestration run.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        command = f"daily-run.{args.action}"
