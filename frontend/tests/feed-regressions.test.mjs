...
test('Feed preserves daily rank across search and discloses its layers on demand', () => {
  ...
  assert.match(feedSource, /mean_voter_position\.toFixed\(6\)/)
  assert.match(feedSource, /author_position\.toFixed\(6\)/)
  assert.match(feedSource, /trusted people and organizations quoted or reposted this Event/)
  assert.match(feedSource, /The original author does not count/)
  assert.match(feedSource, /rank above \$\{meanVoterPercent\}% of the Registry/)
  assert.match(feedSource, /Entities with equal support share the same position/)
  assert.match(feedSource, /first difference from the Event beside it/)
