...
test('Company universe keeps every sourced company eligible for Event matching', () => {
  ...
  assert.doesNotMatch(universe, /Investment context memo/)
  assert.doesNotMatch(universe, /What drives the numbers/)
  assert.match(universe, /Where frontier AI matters/)
  assert.match(universe, /standing bets/)
  assert.doesNotMatch(universe, /pre-registered hypothesis about this company/)
  assert.doesNotMatch(universe, /company-path-driver|affected_driver/)
  assert.match(universe, /company-bet-watch/)
  assert.doesNotMatch(universe, /thesisEffectCopy|thesisStatusCopy|thesis_effect/)
  assert.doesNotMatch(universe, /No attributable public view/)
  assert.doesNotMatch(universe, /top ten`|Last confirmed/)
  assert.match(universe, /memo\.bets\.map/)
  assert.match(universe, /bet\.threshold/)
  assert.match(universe, /open=\{bet\.id === requestedBet\}/)
  assert.match(universe, /target\.scrollIntoView\(\{/)
  assert.match(universe, /target\.focus\(\{ preventScroll: true \}\)/)
  assert.match(universe, /COMPANY_BET_DIRECTION_COPY\[bet\.direction\]/)
  assert.doesNotMatch(universe, /Strategy, dependencies, and research triggers/)
  assert.doesNotMatch(universe, /BIT view and the tests that matter/)
  assert.doesNotMatch(universe, /what_would_support_it|what_would_challenge_it/)
  assert.doesNotMatch(universe, /Source ledger and generation record|cached tokens/)
...
test('Company universe uses the existing flat research-ledger grammar', () => {
  ...
  assert.doesNotMatch(styles, /\.company-memo-intro|\.company-thesis-columns|\.company-path-thesis/)
