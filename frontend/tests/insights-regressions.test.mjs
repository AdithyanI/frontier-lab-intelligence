...
test('Investment Insights expose a minimal company judgment and memo audit', () => {
  ...
  assert.match(apiSource, /mechanism_title: string/)
  assert.match(apiSource, /exposures: InvestmentAgentExposure\[\]/)
  assert.match(apiSource, /materiality: InvestmentAgentMateriality/)
  assert.match(apiSource, /prior_assumption: string \| null/)
  ...
  assert.match(insightSource, /function InvestmentAgentMechanism/)
  assert.match(insightSource, /exposure\.affected_driver/)
  ...
  assert.match(insightSource, /item\.prior_assumption/)
