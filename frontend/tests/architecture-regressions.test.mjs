...
test('Architecture maps routed evidence into the company-aware daily workflow', () => {
  assert.match(architecture, /function EvidenceInputMap/)
  assert.match(architecture, /function DailyIntelligenceMap/)
  assert.match(architecture, /ONE DEVELOPMENT · ONE COMPANY-AWARE RUN/)
  assert.match(architecture, /title: 'Ranked cohort'/)
  assert.match(architecture, /title: '37 company cards'/)
  assert.match(architecture, /title: 'Justified memo calls'/)
  assert.match(architecture, /title: 'Strict output gate'/)
  assert.match(architecture, /title: 'Published cohort'/)
  assert.match(architecture, /A day publishes only when every requested rank succeeds/)
  assert.doesNotMatch(architecture, /Codex App Server|--launch-codex/)
...
test('Architecture gives the daily brief model boundaries visual priority', () => {
  ...
  assert.match(architecture, /task: 'Investment agent'/)
  assert.match(architecture, /model: 'gpt-5\.6-sol'/)
  assert.match(architecture, /screens 37 companies, decides which research memos are worth opening/)
...
test('Architecture explains fallback and resume behavior without changing frozen inputs', () => {
  ...
  assert.match(architecture, /every failed attempt is written to the trace before retry/)
  assert.match(architecture, /No partial cohort replaces the last complete product state/)
