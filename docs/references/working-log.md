...
| 2026-07-08 (test dependency cleanup) | Dobby (Codex Desktop) | Replaced the dev test dependency `httpx` with `httpx2`, matching Starlette's current `TestClient` import path and removing the deprecation warning from `scripts/check-fast.sh` | Codex Desktop (GPT-5) | No API/service spend |
| 2026-07-08 (Digg full follower graph) | Dobby (Codex Desktop) | Added `--full-followers` mode to page Digg's public follower API and ran the full pull: 1,000 rankings, 1,000 profiles, 361,225 directed top-follower edges. Full raw files are local-only under ignored `data/raw/digg-full-2026-07-08/`; tracked summary is `data/digg/full_graph_summary.json`. Kept tracked `data/digg/` snapshot at the smaller first-slice size to avoid huge git artifacts | Codex Desktop (GPT-5), Digg public web/API endpoints | No X API usage and no API/service spend; one known missing profile: `xai` returned 404 |
| 2026-07-08 (build journal contract) | Dobby (Codex Desktop) | Added `docs/references/build-journal.md` and made it a standing AGENTS contract so major chunks capture intent, decisions, evidence, product impact, and next-step rationale. Seeded the journal from existing tracker/log history | Codex Desktop (GPT-5), agent-native-repo-playbook skill | No API/service spend |
