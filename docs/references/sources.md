...
| Digg's own stated methodology (from rankings page "About the rankings" panel, found via full scrape, not TechCrunch this time): "built from the X social graph, using roughly 9 million follow relationships" — confirms algorithmic (follow-graph), not hand-curated | digg.com/tech/x/rankings (page copy, scraped 2026-07-08) | 2026-07-08 | Supersedes earlier "no methodology disclosed" row — this is their own text, not third-party reporting |
| smol.ai/AI News people-tagging whitelist: 33 X handles in `prefPeople`, used to bias their LLM extraction toward known AI voices | github.com/smol-ai/ainews-web-2025/blob/main/oneoffs/preferredTags.ts (pulled via `gh api`) | 2026-07-08 | Public repo, MIT-style OSS; safe to mine directly, no scraping needed |
| Digg profile pages expose initial top-follower rows and total top-follower counts per ranked account; full scrape produced 1,000 ranked accounts and 49,950 initial top-follower edges | digg.com/tech/x/rankings + digg.com/u/x/{handle} | 2026-07-08 | Extracted with `fli digg`; this is a candidate graph, not final identity/affiliation truth |
