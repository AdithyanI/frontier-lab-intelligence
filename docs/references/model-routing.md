...
| Audience Insight generation | `gpt-5.6-terra` | `high` | Quality-first default confirmed by Adi. Investment v9 and Engineering v5 use separate stable cache keys, remain above the prompt-cache threshold, and address BIT's actual readers with distinct internal-research and production-engineering voices. Investment v9 protects the assignment's early researcher-move, strategic-thesis, and attributed capability-observation signals while requiring a public-equity transmission path and a PM/analyst diligence action rather than an Engineering experiment. Across the current twenty-five decisions, seven of fifteen Investment requests each read 1,792 cached tokens (12,544 total), while all ten Engineering requests reported zero reads despite being eligible; cache telemetry remains audience-prefix specific rather than assumed. A Luna/high control produced coherent decisions but zero cache reads across six calls and one less-polished title, so it is not the active default. |
