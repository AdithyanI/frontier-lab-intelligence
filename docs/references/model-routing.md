...
| Audience Insight generation | `gpt-5.6-terra` | `high` | Quality-first default confirmed by Adi. Investment v9 and Engineering v6 use separate stable cache keys, remain above the prompt-cache threshold, and address BIT's actual readers with distinct internal-research and production-engineering voices. Investment v9 protects the assignment's early researcher-move, strategic-thesis, and attributed capability-observation signals while requiring a public-equity transmission path and a PM/analyst diligence action rather than an Engineering experiment. Engineering v6 keeps concrete early capability and architecture signals while translating specialized source patterns into team-relevant investigations. The current 173-decision store read 242,176 cached input tokens and records $3.622624 in proxy-reported cost. The fresh July 14–15 top-30 cohort made 98 decisions, read 152,576 cached tokens, and cost $2.317567; cache telemetry remains audience-prefix specific rather than assumed. A Luna/high control produced coherent decisions but zero cache reads across six calls and one less-polished title, so it is not the active default. |
