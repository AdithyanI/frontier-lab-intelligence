...
| `@adithyan_ai` followed 767 accounts at snapshot time; a one-time 2,000-follower privacy/noise floor retained 638. | TwitterAPI.io `GET /twitter/user/followings`, source profile `https://x.com/adithyan_ai/following` | 2026-07-10 | Current `adi_following`: 638 facts/edges, all with follower counts and 628 with bios. Cleanup removed 129 lower-follower links and 127 Adi-only accounts, preserved two accounts with other evidence, and hid Adi's source account from the Registry. Remaining overlap: 177 Digg-ranked, 126 AI High Signal, 10 smol.ai, 256 prior-graph. This was an explicit one-time cleanup, not an importer policy. |
