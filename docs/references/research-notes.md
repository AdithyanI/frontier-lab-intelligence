...
| TwitterAPI.io | Followers/following: from `$0.01 / 1K`, tiered; profiles `$0.18 / 1K`. | Used successfully for AI High Signal list-member import after Adi added a `$10` top-up. API key is a machine-local file at `~/.secrets/twitterapi-io/api-key`; list-member importer uses `X-API-Key` and pages until the provider stops. |
