...
| The `$99` TwitterAPI.io Builder subscription documents a 10-QPS cap. | `https://twitterapi.io/subscribe` | 2026-07-11 | The bounded profile scan used 10 workers with request starts limited to 9 QPS; 2,039 remaining network requests completed in about four minutes without rate-limit failure. |
| A current profile scan resolved 2,228 of 2,231 frozen sources and projects the complete outgoing-follow crawl at `$27.83826`. | Local `registry-following-2026-07-11-v1` snapshot + TwitterAPI.io user profiles | 2026-07-11 | Nine sources are protected, three are missing, and 12 advertise following=0. High-following outliers are a visible cost feature, not an automatic rejection rule. |
| The completed fresh snapshot contains 2,456,305 directed edges over 463,180 targets; 290,408 targets (62.7%) appear once, while 69,118 appear at least five times. | Immutable local `registry-following-2026-07-11-v1` snapshot | 2026-07-11 | Keep the complete edge list as evidence, but rank and visualize bounded derived projections. A full-graph browser rendering would mostly expose one-off noise rather than explain the useful signal. |
| TwitterAPI.io does not expose a protected account's outgoing follows. | Bounded `@alsuhr` following probe, `pageSize=20` | 2026-07-11 | The provider returned success with zero rows/no cursor despite the profile advertising 637 follows. Treat protected as inaccessible, never as an empty graph. Protected identities may still receive inbound edges; Karpathy follows protected `@dwf` and `@gwern`. |
| Protected-only X identities belong in the reversible Rejected set, not active collection or ranking. | Adi decision after profile scan and live probe | 2026-07-11 | Nine identities have reason `protected_x_no_public_channel`. The live Registry now exposes 23 rejected rows across all reason codes; `clear_rejection` is the restoration path. |
...
| --- | --- | --- | --- | --- |
| 1 | Trusted-person X following snapshots | Graph/ranking | Pull who reviewed trusted X channels follow; bounded, high-signal attention graph. | Complete immutable snapshot: 2,231 sources, 2,456,305 directed edges, 463,180 targets. Ranking remains next. |
