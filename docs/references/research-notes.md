...
| Protected-only X identities belong in the reversible Rejected set, not active collection or ranking. | Adi decision after profile scan and live probe | 2026-07-11 | Nine identities now have reason `protected_x_no_public_channel`. The live Registry exposes 13 rejected rows with reasons; `clear_rejection` is the only restoration path. |
