...
| Feed API | `src/fli/web/feed.py`, `GET /api/events?date=…&sort=attention` | Daily score under the `attention-v1.1` contract: 100×(0.55 tracked amplification + 0.25 author support + 0.20 public engagement), each a within-day percentile. The UI leads with a stable all-evidence daily rank; Audit/search filters never restart it. The internal envelope sort key remains `peak_attention_score`. |
