...
- [ ] Bump `SCHEMA_VERSION` to `audience-insight-output-v4`.
- [ ] Rename output fields: `_OUTPUT_FIELDS`, `OUTPUT_FORMAT` (name
...
- [ ] Note: the action field differs per audience (`watchpoint` vs
...
- [ ] Update `PROMPT_CONTRACTS`: versions `investment-insight-v10` /
      `ai-engineering-insight-v7`, new paths, cache keys
      `fli:insights:investment:v10` / `fli:insights:ai-engineering:v7`.
- [ ] `render_input`: emit `evaluation_day:` header and per-post `posted:`
...
- [ ] Rename `implication`/`next_step` columns in `SCHEMA`, INSERT/UPDATE
      ...
      already a key).
- [ ] Old `insights.db` rows are all invalidated by the version bump and the
...
- [ ] `src/fli/web/insights.py` `_item_payload`: emit `why_it_matters` and
      `watchpoint`/`experiment` (or a generic `action` + `action_label`);
      `decision_reason` for kept items now reads `why_it_matters`.
- [ ] `frontend/src/api.ts` types + `frontend/src/pages/Insights.tsx`
      labels: "Why it matters", "Watchpoint" (investment), "Experiment"
      (engineering). Then `npm --prefix frontend run build`.

Validation:

- [ ] Update/extend existing insight tests for new field names and
      per-audience schema; `scripts/check-fast.sh`.
- [ ] One-envelope dry run per audience (`fli insights` freeze + inspect
      request JSON: dates present, prompt v10/v7, cache key new) BEFORE any
      model call.
- [ ] Verify the 2026-07-15 r44 ChatGPT envelope now renders with its 2022
      `posted:` date in the frozen request.

Run (needs Adi go-ahead; ~$4 expected):

- [ ] Regenerate all routed-positive envelopes under v10/v7.
- [ ] Spot-audit: stale-source suppression fired on r44; tickers named where
