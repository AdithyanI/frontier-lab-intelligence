...
| todo | **C. Layer curated lists:** smol.ai `prefPeople` first (then swyx / Anthropic staff lists) as `account_source_facts` rows joined on handle; triangulation across sources drives candidate confidence. | Generic X-list importer added: `fli sources import-x-list --list-id ... --source ...`. AI High Signal imported 2026-07-09 via TwitterAPI.io: 609 members; 230 already in Digg, 379 new versus Digg. Next: import smol `prefPeople` or Anthropic staff, then classify. |
