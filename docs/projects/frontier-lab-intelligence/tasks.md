...
| todo | **F. Labs as first-class entities:** `labs` table hand-seeded (~10 labs from the PDF list incl. SSI + Thinking Machines) with official channels (org X handle → `accounts` link, blog, GitHub org, arXiv affiliation). | Registry design agreed with Adi 2026-07-09: labs hand-curated by judgment; people graph-derived; affiliation person→lab optional (independents are valid). All major lab org accounts already exist in the Digg graph (verified: openai, anthropicai, xai, ssi, thinkymachines, …). |
