...
causally plausible company memos, and persists minimal, auditable company
read-throughs. A Sol/xhigh calibration across the July 21 top ten now proves
both surface and suppress paths; it is not yet a completed full-day replay.
...
| Feed audience routing | Luna/medium Development top-100 validated | The complete historical July 5–21 `daily-rank-v2` Event runs remain auditable. The self-contained `audience-routing-v13` prompt now routes one whole Development packet with all original posts and artifact lineage while leaving pure amplifiers out of semantic evidence. The sequential July 21 top-100 pass completed all 97 routable Developments with zero failures: 55 both, 10 Engineering-only, 11 Investment-only, and 21 neither. Three displayed ranks lacked a current first-party X source from which to freeze a packet. A positive route is a recall-oriented candidate for downstream investigation, not a publishable Insight; the ten highest-ranked Developments have now completed successor Investment analysis. |
| Audience Insight generation | Company-aware Investment top-ten calibration | Historical Investment v10 and AI Engineering v7 outputs remain preserved, and current readers reject their superseded Event-rank lineage as successor output. A Sol/xhigh two-stage pass across the July 21 top ten screened all 37 companies per Development, surfaced six Developments, suppressed four, opened 19 company memos, retained 18 assessments, and rejected one candidate after memo review. Every final result now includes a short investment-facing headline written after company analysis. A subsequent v8 rank-one calibration makes the Development summary lead with what changed and omit a redundant date unless chronology needs it; only rank one has been rerun under that instruction pending human review. The original top-ten batch reused 77,824 cached input tokens and reported $3.026857 total model cost; the v8 rank-one rerun reported $0.370646. The decisions and plain-language output are persisted under `investment-agent-read-v3`. Full-day Investment replay and an AI Engineering successor remain unproven. |
...
| Investment company context | Complete 37-company research set plus top-ten mapping proof | BIT Lens and the Investment read path expose a source-bearing memo for every company in the canonical candidate universe. Each packet separates company prior context from Event evidence and includes business economics, operating drivers, testable frontier-AI transmission paths, thesis tests, uncertainties, and an exact dated source ledger. The two-stage agent now proves structured Event-to-company judgment across the July 21 top ten, including reason-bearing suppression and after-memo rejection; the next boundary is replay across the routed cohort, not company cold start. |
