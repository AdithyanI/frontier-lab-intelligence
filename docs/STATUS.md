...
| Submission package | Live product and reproducible local client complete; external package pending | The public hosted product is the primary reviewer path. A clean checkout also restores a checksummed read-only snapshot with one command. The rubric-mapped write-up and final delivery review remain. Nothing has been submitted to BIT. |
