...
| Feed audience routing | Luna/medium Development top-100 complete for July 5–21 | The complete historical July 5–21 `daily-rank-v2` Event runs remain auditable. The self-contained `audience-routing-v14` prompt routes one whole Development packet with all original posts and artifact lineage, leaves pure amplifiers out of semantic evidence, and requires Investment relevance to follow from the central Development rather than an incidental fact. The current 17-day pass completed all 1,647 routable Developments with zero failures: 612 both, 219 Engineering-only, 96 Investment-only, and 720 neither. It reused 2,791,936 cached tokens from 6,990,192 input tokens and reported $7.079129. A positive route is a recall-oriented candidate for downstream investigation, not a publishable Insight; the ten highest-ranked Investment-positive Developments on July 19–21 have completed successor Investment analysis. |
...
and `daily-development-rank-v1` provides the common order. Development routing
is complete for the July 19–21 top 100, and the Investment successor is
published for the ten highest positive Investment routes on each day. The next
step is qualitative calibration of those 30 decisions, then an equivalent
current-lineage AI Engineering contract. Editorial, PDF, and delivery replay
remain deferred until both per-Development audience boundaries are credible.
