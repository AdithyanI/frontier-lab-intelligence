...
| Feed audience routing | Implemented; broad audit surface ready | The direct Evidence runtime returns independent AI Engineering and Investment booleans plus evidence-grounded reasons, preserves the attributed primary/artifact/reaction hierarchy, and stores exact packet/input/model/cost provenance. It consumes only successful artifact text and contains no artifact-quality heuristic. The complete packet remains immutable; only the model-facing view is capped at 20,000 tokens with an explicit truncation marker, which affected 16/900 current packets. GPT-5.4 mini at high reasoning completed the top 100 envelopes for every complete day from July 5–13: 357 `both`, 99 Engineering-only, 147 Investment-only, and 297 `neither`. The stable prompt key produced 814 cache-hit requests and 1,463,296 cached tokens from 3,634,944 input tokens; all 900 responses reported cost, totaling $5.506914. Twenty-eight repeated exact inputs had zero label conflicts. A prior contextual audit established the narrow Investment uncertainty and Engineering access/limit rules. Exact snapshot matches appear in Feed as `ENG`/`INV` marks with both reasons in one disclosure, while the derived Status control exposes Relevant, Not relevant, and Not evaluated without inventing a third judgment. |
