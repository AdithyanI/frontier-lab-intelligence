...
| Daily editorial agent | Complete eleven-day corpus; workspace v3 contract active | `$fli-daily-intelligence` freezes the routed-positive day, applies the seven-day first-party X window, projects authoritative X publication dates, and exposes exact artifact disclosure lineage without automatically filtering artifacts. New workspaces use Event-native semantic snapshot names under v3; the existing July 5–15 v2 workspaces and imported corpus remain immutable provenance. The agent audits artifact timing and relevance; artifact citations require excerpts verified against frozen artifact text. Jul 14 calibration excludes four old-only candidates and 53 stale X sources. Paper Glider remains inspectable but is marked by its Jul 15 disclosure and must not be used as Jul 14 evidence. The Jul 10 Thinking Machines canary correctly retains Mira Murati's current quote/reply and removes its 2025 root. Embeddings remain optional retrieval only. |
