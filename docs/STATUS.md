...
| Feed audience routing | v15 evidence gate implemented; v14 July 5–21 proof preserved | The complete historical v14 top-100 pass remains auditable: 1,647 Developments, zero failures, 612 both, 219 Engineering-only, 96 Investment-only, and 720 neither. The v15 freeze contract now completes a Development without an LLM call only when its complete packet is one X post with at most 30 substantive words and no artifact, author continuation, or independently authored corroboration. Both audiences receive an explicit stored negative reason, and run telemetry separately counts short unsupported text and unavailable linked/media evidence. A dry audit found 122 and 203 respectively; no v15 production replay has relabeled the immutable v14 results. |
...
and `daily-development-rank-v1` provides the common order. The historical v14
Development routing proof is complete for July 5–21, and the Investment
successor is published for the ten highest positive Investment routes on July
19–21. The v15 evidence-readiness gate is implemented but has not yet been
replayed into production runs. The next step is that replay followed by
qualitative calibration, then an equivalent current-lineage AI Engineering
