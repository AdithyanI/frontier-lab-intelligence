...
- Use the `$project` skill for tracker mechanics (location, workflow, archive); this repo uses its defaults. Pre-split history: `docs/projects/archive/frontier-lab-intelligence/`.
...
2. `docs/projects/<project>/tasks.md` (active projects) — current plan, status, blockers, and validation evidence.
...
- `docs/projects/<project>/tasks.md` trackers are active execution state only; archive per the `$project` skill.
